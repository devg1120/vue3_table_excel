...
export default defineComponent({
  ...
  methods: {
    ...
    rowSepMouseDown (e) {
      e.preventDefault()
      e.stopPropagation()
      if (this.allowAddCol && !e.target.classList.contains('row-sep')) {
        e.target.style.display = 'none'
        const me = e.target.parentElement.parentElement
        const pos = Array.from(me.parentElement.children).findIndex(td => td === me)
        this.insertColumn(pos)
      ...
      //}
      this.sep = {}
      this.sep.curRow = e.target.parentElement.parentElement;
      this.sep.pageY = e.pageY
      let padding = 0
	    /*
      if (getStyleVal(this.sep.curCol, 'box-sizing') !== 'border-box') {
        const padLeft = getStyleVal(this.sep.curCol, 'padding-left')
	    ...
	    */
      this.sep.curRowHeight = e.target.parentElement.offsetHeight - padding
      // if (this.sep.nxtCol)
      //   this.sep.nxtColWidth = this.sep.nxtCol.offsetWidth - padding
      window.addEventListener('mousemove', this.rowSepMouseMove)
      window.addEventListener('mouseup', this.rowSepMouseUp)
    },
    rowSepMouseOver (e) {
      if (e.target.classList.contains('row-sep')) {
        //e.target.style.borderRight = '5px solid #cccccc'
        //e.target.style.height = this.systable.getBoundingClientRect().height + 'px'
        //if (this.allowAddCol)
        console.log(this.allowAddCol)
        //  e.target.children[0].style.display = 'block'
        e.target.style.borderBottom = '5px solid #cccccc'
        e.target.style.width = this.systable.getBoundingClientRect().width + 'px'
        //if (this.allowAddCol)
        //   e.target.children[0].style.display = 'block'
      }
      else {
    ...
    rowSepMouseMove (e) {
      if (!this.sep || !this.sep.curRow) return
      const diffY = e.pageY - this.sep.pageY
      // this.sep.curCol.style.width = (this.sep.curColWidth + diffX) + 'px'
      const newHeight = (this.sep.curRowHeight + diffY) + 'px'
      this.sep.curRow.style.height = newHeight
      //this.sep.curField.height = newHeight
      this.lazy(this.calStickyLeft, 200)
    },
    rowSepMouseUp (e) {
      e.preventDefault()
    ...
    */
    ...
    insertRecord (rec, index, selectAfterDone, noLastPage, isUndo) {
      console.log("insertRecord.....", index);
      if (typeof rec === 'undefined') rec = {}
      this.fields.map(f => {
        if (typeof rec[f.name] === 'undefined') {
          if (f.keyField)
      ...
      this.modelValue.splice(index,0,rec)  // GUSA
      this.table.splice(index,0,rec)       // GUSA
      const rowPos = index + 1;    //GUSA
        let new_selected = {};                //GUSA
        for (let key in this.selected) {
             new_selected[parseInt(key)+1] = this.selected[key];
